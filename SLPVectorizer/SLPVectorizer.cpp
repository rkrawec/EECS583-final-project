...
static cl::opt<bool>
    ShouldVectorizeHor("slp-vectorize-hor-Ryan-edit", cl::init(true), cl::Hidden,
                       cl::desc("Attempt to vectorize horizontal reductions"));

static cl::opt<bool> ShouldStartVectorizeHorAtStore(
    "slp-vectorize-hor-store-Ryan-edit", cl::init(false), cl::Hidden,
    cl::desc(
        "Attempt to vectorize horizontal reductions feeding into a store"));

static cl::opt<int>
    MaxVectorRegSizeOption("slp-max-reg-size-Ryan-edit", cl::init(128), cl::Hidden,
                           cl::desc("Attempt to vectorize for this register size in bits"));

static cl::opt<unsigned>
    MaxVFOption("slp-max-vf-Ryan-edit", cl::init(0), cl::Hidden,
                cl::desc("Maximum SLP vectorization factor (0=unlimited)"));

static cl::opt<int>
    MaxStoreLookup("slp-max-store-lookup-Ryan-edit", cl::init(32), cl::Hidden,
...
static cl::opt<int>
    ScheduleRegionSizeBudget("slp-schedule-budget-Ryan-edit", cl::init(100000), cl::Hidden,
                             cl::desc("Limit the size of the SLP scheduling region per block"));

static cl::opt<int> MinVectorRegSizeOption(
    "slp-min-reg-size-Ryan-edit", cl::init(128), cl::Hidden,
    cl::desc("Attempt to vectorize for this register size in bits"));

static cl::opt<unsigned> RecursionMaxDepth(
    "slp-recursion-max-depth-Ryan-edit", cl::init(12), cl::Hidden,
    cl::desc("Limit the recursion depth when building a vectorizable tree"));

static cl::opt<unsigned> MinTreeSize(
    "slp-min-tree-size-Ryan-edit", cl::init(3), cl::Hidden,
    cl::desc("Only vectorize small trees if they are fully vectorizable"));

// The maximum depth that the look-ahead score heuristic will explore.
// The higher this value, the higher the compilation time overhead.
static cl::opt<int> LookAheadMaxDepth(
    "slp-max-look-ahead-depth-Ryan-edit", cl::init(2), cl::Hidden,
...
namespace llvm
{
  ...
  namespace slpvectorizer
  {
    ...
    public:
      ...
      {
        unsigned MaxVF = MaxVFOption.getNumOccurrences() ? MaxVFOption : TTI->getMaximumVF(ElemWidth, Opcode);
  ...
  template <>
  struct DOTGraphTraits<BoUpSLP *> : public DefaultDOTGraphTraits
  {
    using TreeEntry = BoUpSLP::TreeEntry;
                            ...
                            const EdgeInfo &UserTreeIdx)
{
  ...
  unsigned ShuffleOrOp = S.isAltShuffle() ? (unsigned)Instruction::ShuffleVector : S.getOpcode();
  switch (ShuffleOrOp)
  {
  ...
  case Instruction::InsertElement:
  {
    ...
    if (count_if(VL, [&SourceVectors](Value *V)
                 { return !SourceVectors.contains(V); }) >= 2)
    {
                   ...
                   TargetTransformInfo *TTI, TargetLibraryInfo *TLI)
{
  ...
  auto Shape = VFShape::get(*CI, ElementCount::getFixed(static_cast<unsigned>(VecTy->getNumElements())),
                            false /*HasGlobalPred*/);
  Function *VecFunc = VFDatabase(*CI).getVectorizedFunction(Shape);
...
InstructionCost BoUpSLP::getTreeCost(ArrayRef<Value *> VectorizedVals)
{
  ...
  if (FirstUsers.size() == 1)
  {
    ...
    if (all_of(ShuffleMask.front(), [Limit](int Idx)
               { return Idx < Limit; }) &&
...
Value *BoUpSLP::vectorizeTree(TreeEntry *E)
{
  ...
  case Instruction::Call:
  {
    ...
    {
      VFShape Shape =
          VFShape::get(*CI, ElementCount::getFixed(static_cast<unsigned>(VecTy->getNumElements())),
                       false /*HasGlobalPred*/);
